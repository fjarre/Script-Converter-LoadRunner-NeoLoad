...
public class LauncherTest {
	...
	public void mainTestHelp() {
		Assert.assertEquals(0, Launcher.run(ImmutableList.of("--help").toArray(new String[0])));
	}

	@Test
	public void mainTestH() {
		Assert.assertEquals(0, Launcher.run(ImmutableList.of("-h").toArray(new String[0])));
	}

	@Test
	public void mainTestInvalid() {
		Assert.assertEquals(1, Launcher.run("-p test".split(" ")));
	}
	...
	public void mainTestNoLrProjectWithLogs() {
		...
		Assert.assertEquals(2, Launcher.run(args.split(" ")));
	}
	...
	public void mainTestInvalidCustomMapping() throws IOException {
		...
		Assert.assertEquals(1, Launcher.run(args.split(" ")));
	}
	...
	public void mainTestInvalidProjectFile() {
		Assert.assertEquals(1, Launcher.run("-s mySource -t src/test/resources/projects -p test".split(" ")));
	}
